...
export default function QuizPartyGame() {
  const [view, setView] = useState('home'); // home, hostLogin, playerLogin, hostCreate, hostWaiting, hostRound2, hostResults, playerWaiting, playerRound1, playerRound2
  ...
  const loadGameData = async (gId) => {
    ...
    try {
      ...
      if (game) {
        ...
        const participantPromises = participantIds.map(id => storageGet(`participant:${gId}:${id}`));
        const participants = await Promise.all(participantPromises);
        
        setGameData(prev => ({ ...prev, participantList: participants.filter(p => p) }));
      }
    } catch (err) {
  ...
  const createGame = async () => {
    ...
    const game = {
      ...
      status: 'waiting', // waiting, round1, round2, completed
      participants: [hostId],
    ...
    const success = await storageSet(`game:${newGameId}`, game);
    if (success) {
  ...
  const joinGame = async () => {
    ...
    const success = await storageSet(`game:${gameId}`, game);
    if (success) {
  ...
  const submitRound1 = async () => {
    if (answers.length !== gameData.questions.length) {
    ...
    await storageSet(`round1:${gameId}:${participantId}`, round1Data);
    setView('playerWaiting');
    setAnswers([]);
    await loadGameData(gameId);
    setLoading(false);
  ...
  // Check if all participants completed round 1
  const checkRound1Complete = async () => {
    if (!gameData) return false;
    
    const checks = await Promise.all(
      gameData.participants.map(pid => storageGet(`round1:${gameId}:${pid}`))
    );
    
    return checks.every(c => c !== null);
  };

  // Start Round 2 - assign questions randomly
  const startRound2 = async () => {
    ...
    // Create assignments - each participant gets questions about different people
    const assignments = {};
    
    for (const pid of participants) {
      assignments[pid] = [];
      
      for (let qIdx = 0; qIdx < game.questions.length; qIdx++) {
        // Get list of participants excluding current one
        const otherParticipants = participants.filter(p => p !== pid);
        
        // Randomly assign one
        const randomIdx = Math.floor(Math.random() * otherParticipants.length);
        const assignedTo = otherParticipants[randomIdx];
  ...
  const submitRound2 = async () => {
    ...
    await storageSet(`round2:${gameId}:${participantId}`, round2Data);
    setView('playerWaiting');
    setAnswers([]);
    await loadGameData(gameId);
    setLoading(false);
  ...
  // Check if all participants completed round 2
  const checkRound2Complete = async () => {
    if (!gameData) return false;
    
    const checks = await Promise.all(
      gameData.participants.map(pid => storageGet(`round2:${gameId}:${pid}`))
    );
  ...
  // Get assigned participant name
  const getAssignedName = async (qIdx) => {
    const assignments = await storageGet(`round2assignments:${gameId}`);
    if (!assignments || !assignments[participantId]) return '';
    
    const assignment = assignments[participantId].find(a => a.questionIndex === qIdx);
  ...
  // Poll for game updates and check round completion
  useEffect(() => {
    if (view === 'playerWaiting' || view === 'hostWaiting') {
      const interval = setInterval(async () => {
        await loadGameData(gameId);
        
        // Check if game started for players
        const game = await storageGet(`game:${gameId}`);
        if (game && view === 'playerWaiting') {
        ...
        // Check if rounds are complete for host
        if (game && isHost && view === 'hostWaiting') {
          if (game.status === 'round1') {
            const round1Complete = await checkRound1Complete();
            if (round1Complete) {
              setView('hostRound2');
            }
          } else if (game.status === 'round2') {
            const round2Complete = await checkRound2Complete();
            if (round2Complete) {
              await viewResults();
            }
          }
    ...
    useEffect(() => {
      const loadResults = async () => {
        ...
        for (const pid of game.participants) {
          ...
          // Get round 2 with assignments
          if (round2 && assignments[pid]) {
            for (let i = 0; i < round2.answers.length; i++) {
              const assignment = assignments[pid][i];
              const assignedParticipant = await storageGet(`participant:${gameId}:${assignment.assignedParticipantId}`);
              
              playerResult.round2Answers.push({
                answer: round2.answers[i],
                aboutPerson: assignedParticipant.name
              });
            }
          }
        ...
        {resultsData.results.map((player, idx) => (
          <div key={idx} className="player-results">
            ...
            <div className="round-section">
              ...
              {resultsData.game.questions.map((q, qIdx) => (
                <div key={qIdx} className="result-item">
                  <div className="question">{q} <span className="about">(about {player.round2Answers[qIdx]?.aboutPerson})</span></div>
    ...
    <div className="app">
      ...
      {view === 'home' && (
        <div className="container">
          ...
          <button onClick={() => setView('hostLogin')}>
            <Users size={20} />
            Create Game (Host)
          </button>

          <button onClick={() => setView('playerLogin')} className="btn-secondary">
            <LogIn size={20} />
            Join Game (Player)
          </button>
        </div>
      ...
      {view === 'hostLogin' && (
        <div className="container">
          <div className="header">
            ...
            <p className="subtitle">Set up questions for your party</p>
          </div>
          ...
          <button onClick={() => { setView('home'); setError(''); }} className="btn-secondary">
            Back
          </button>
        </div>
      )}
      ...
      {view === 'playerLogin' && (
        <div className="container">
          <div className="header">
            <h1>Join a Game</h1>
            <p className="subtitle">Enter the game code from your host</p>
          </div>
      ...
      {view === 'hostWaiting' && gameData && (
        <div className="container">
          ...
          <div className="participants-list">
            <h3>
              <Users size={20} />
              Players ({gameData.participantList?.length || 0})
            </h3>
          ...
          <button
            onClick={startRound1}
            disabled={!gameData.participantList || gameData.participantList.length < 2}
          >
            <Play size={20} />
            Start Round 1
          </button>
        ...
        <div className="container">
          <div className="header">
            ...
            <p className="subtitle">
              ...
              {gameData.status === 'completed' && 'Game complete! Waiting for host to share results...'}
            </p>
          </div>
      ...
      {view === 'playerRound1' && gameData && (
        <div className="container">
          <div className="header">
            <h1>Round 1</h1>
            <p className="subtitle">Answer questions about yourself</p>
          </div>

          <div className="status-badge">
            <FileText size={16} />
            Question {answers.length + 1} of {gameData.questions.length}
          </div>
          ...
          <button onClick={submitRound1} disabled={loading}>
            <Send size={20} />
      ...
      {view === 'playerRound2' && gameData && (
        <div className="container">
          <div className="header">
            <h1>Round 2</h1>
            <p className="subtitle">Answer questions about others</p>
          </div>
          ...
          <div className="answer-form">
            {gameData.questions.map((q, idx) => {
              ...
              useEffect(() => {
                getAssignedName(idx).then(setAssignedName);
              }, [idx]);
          ...
          <button onClick={submitRound2} disabled={loading}>
            <Send size={20} />
            {loading ? 'Submitting...' : 'Submit Answers'}
          </button>
        </div>
      ...
      {view === 'hostRound2' && gameData && (
        <div className="container">
          <div className="header">
            <h1>Ready for Round 2?</h1>
            <p className="subtitle">All players have completed Round 1</p>
          </div>
